...
状态：历史完成记录（P1-S0 至 P1-S10）

> 本文档于 2026-08-03 从 Git 历史恢复，用于重新审视三阶段算子规划与
> OpenCV 模块覆盖差距。它记录第一阶段当时的实施和验收过程；当前支持范围、
> target 名称和测试状态以
> [API coverage](opencv-core-imgproc-api-coverage.md) 及当前架构文档为准。
...
所有行为定义以固定的 upstream OpenCV 源码为准：
...
- [x] `cvh::headers` 在优化开启与关闭时保持同一套 API 集合。
