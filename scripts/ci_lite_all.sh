...
BUILD_DIR="${ROOT_DIR}/build-ci-lite-all"

print_env_fingerprint() {
  echo "ci_lite_env_begin"
  echo "uname: $(uname -a)"
  echo "compiler: $(c++ --version | head -n 1)"
  echo "cmake: $(cmake --version | head -n 1)"
  echo "python: $(python3 --version)"
  echo "ci_lite_env_end"
}
...
echo "ci_lite_cmake_cache_begin"
...
echo "ci_lite_cmake_cache_end"

cmake --build "${BUILD_DIR}" --target cvh_test_core_lite cvh_test_imgproc -j

run_gtest_with_case_log "${BUILD_DIR}/cvh_test_core_lite" "cvh_test_core_lite"
run_gtest_with_case_log "${BUILD_DIR}/cvh_test_imgproc" "cvh_test_imgproc_lite"
