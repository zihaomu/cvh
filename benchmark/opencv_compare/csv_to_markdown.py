...
def row_algorithm(row: dict) -> str:
    return (
        (row.get("algorithm_path", "") or "").strip()
        or (row.get("dispatch_path", "") or "").strip()
        or "unknown"
    )


def row_dispatch(row: dict) -> str:
    return (row.get("dispatch_path", "") or "").strip() or "unknown"


def row_isa(row: dict) -> str:
...
def render_report(rows, title: str, input_path: Path, meta_path: Optional[Path] = None) -> str:
    ...
    match_template_has_ui = any(
        ...
        and row_dispatch(r) == "opencv_ui"
        for r in supported
    )
    histogram_has_fastpath = any(
        r.get("op", "") in {"CALC_HIST", "COMPARE_HIST"}
        and row_algorithm(r) == "header_fastpath"
        for r in supported
    )
    ...
    for suite in ("core_mat", "imgproc"):
        ...
        for op in op_names:
            ...
            algorithms = sorted({row_algorithm(r) for r in suite_rows if r.get("op", "") == op})
            dispatches = sorted({row_dispatch(r) for r in suite_rows if r.get("op", "") == op})
            isas = sorted({row_isa(r) for r in suite_rows if r.get("op", "") == op})
            ratio = geometric_mean(values)
