...
TEST(ParallelForTest, set_num_threads_is_clamped)
{
...
TEST(ParallelForTest, single_thread_runs_serial)
{
    ParallelRuntimeScope scope;
    cvh::setNumThreads(1);
...
TEST(ParallelForTest, stdthread_backend_covers_each_index_once)
{
    ParallelRuntimeScope scope;
    cvh::setNumThreads(4);
    cvh::setParallelBackend(cvh::ParallelBackend::StdThread);
...
TEST(ParallelForTest, serial_backend_ignores_requested_stripes)
{
    ParallelRuntimeScope scope;
    cvh::setNumThreads(8);
...
TEST(ParallelForTest, openmp_backend_falls_back_when_unavailable)
{
    ParallelRuntimeScope scope;
    cvh::setNumThreads(4);
...
TEST(ParallelForTest, worker_exception_propagates)
{
    ParallelRuntimeScope scope;
    cvh::setNumThreads(4);
    cvh::setParallelBackend(cvh::ParallelBackend::StdThread);
...
TEST(ParallelForTest, parallel_loop_body_overload_matches_expected_sum)
{
    ParallelRuntimeScope scope;
    cvh::setNumThreads(4);
    cvh::setParallelBackend(cvh::ParallelBackend::StdThread);
