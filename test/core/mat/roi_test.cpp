...
TEST(MatRoiTest, colrange_is_view_and_non_continuous)
{
    Mat src({3, 5}, CV_32S);
...
TEST(MatRoiTest, clone_from_non_continuous_roi_is_continuous_and_deep_copy)
{
    Mat src({3, 5}, CV_32S);
    int* src_data = reinterpret_cast<int*>(src.data);
    for (int r = 0; r < 3; ++r)
...
TEST(MatRoiTest, copyto_supports_non_continuous_dst_roi)
{
    Mat src({3, 5}, CV_32S);
    int* src_data = reinterpret_cast<int*>(src.data);
    for (int r = 0; r < 3; ++r)
...
TEST(MatRoiTest, convertto_from_non_continuous_multichannel_roi)
{
...
TEST(MatRoiTest, range_operator_creates_view)
{
    Mat src({4, 5}, CV_32S);
    int* src_data = reinterpret_cast<int*>(src.data);
    for (int i = 0; i < 20; ++i)
