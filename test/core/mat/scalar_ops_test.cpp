...
TEST(MatScalarOpsTest, setto_scalar_fills_all_channels_per_pixel)
{
    Mat m({2, 2}, CV_8UC3);
...
TEST(MatScalarOpsTest, operator_assign_scalar_matches_setto_behavior)
{
    Mat m({1, 3}, CV_32SC2);
    m = Scalar(10.0, -7.0);
...
TEST(MatScalarOpsTest, setto_scalar_supports_non_continuous_roi)
{
...
TEST(MatScalarOpsTest, setto_scalar_rejects_more_than_four_channels)
{
    Mat m({1, 1}, CV_8UC(5));
    EXPECT_THROW(m.setTo(Scalar::all(1.0)), Exception);
}

TEST(MatScalarOpsTest, setto_scalar_saturates_values_like_float_path)
{
    Mat m({1, 2}, CV_8UC4);
...
TEST(MatScalarOpsTest, add_mat_scalar_and_scalar_mat_support_multichannel)
{
...
TEST(MatScalarOpsTest, subtract_mat_scalar_and_scalar_mat_support_multichannel)
{
    Mat src({2, 2}, CV_32SC3);
    int seed = 1;
    for (int y = 0; y < 2; ++y)
...
TEST(MatScalarOpsTest, add_subtract_mat_scalar_and_scalar_mat_support_float32_c1)
{
...
TEST(MatScalarOpsTest, multiply_divide_mat_scalar_and_scalar_mat_support_float32_c1)
{
    Mat src({2, 3}, CV_32FC1);
    float seed = 0.5f;
    for (int y = 0; y < 2; ++y)
...
TEST(MatScalarOpsTest, compare_mat_scalar_and_scalar_mat_return_u8_mask_with_channels)
{
    Mat src({2, 2}, CV_32SC3);
...
TEST(MatScalarOpsTest, compare_fp16_scalar_paths_work_without_openmp_requirement)
{
...
TEST(MatScalarOpsTest, scalar_binary_and_compare_support_non_continuous_roi)
{
...
TEST(MatScalarOpsTest, scalar_binary_and_compare_reject_more_than_four_channels)
{
...
TEST(MatScalarOpsTest, mat_mat_binary_and_compare_still_support_more_than_four_channels)
{
    Mat a({1, 2}, CV_32SC(5));
    Mat b({1, 2}, CV_32SC(5));
...
TEST(MatScalarOpsTest, mat_mat_non_continuous_roi_still_supports_more_than_four_channels)
{
