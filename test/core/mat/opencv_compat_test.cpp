...
TEST(MatOpenCVCompatTest, metadata_helpers_match_continuous_layout)
{
    Mat m({2, 3, 4}, CV_16U);
...
TEST(MatOpenCVCompatTest, create_same_shape_and_type_keeps_existing_buffer)
{
    Mat m({2, 2}, CV_32F);
...
TEST(MatOpenCVCompatTest, copyto_preallocated_buffer_reuses_storage)
{
    Mat src({2, 3}, CV_32S);
...
TEST(MatOpenCVCompatTest, release_then_recreate_remains_usable)
{
    Mat m({2, 2}, CV_8U);
    m.setTo(7.0f);
    m.release();
