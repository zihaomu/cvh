...
TEST(MatLayoutTest, interleaved_address_formula_matches_pixelptr_and_at)
{
    Mat m({2, 3}, CV_8UC3);
    uchar* raw = m.ptr();
    for (int i = 0; i < 18; ++i)
...
TEST(MatLayoutTest, roi_uses_parent_stride_and_preserves_address_formula)
{
    Mat src({4, 5}, CV_8UC3);
    for (int y = 0; y < 4; ++y)
    {
...
TEST(MatLayoutTest, chw_reorder_roundtrip_matches_interleaved_for_contiguous_and_roi)
{
