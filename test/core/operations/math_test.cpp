#include "test/core/support/math_test_utils.hpp"

TEST(MathTest, scale_add_supports_multichannel_and_in_place)
{
    Mat a({1, 5}, CV_8UC3);
    Mat b({1, 5}, CV_8UC3);
    for (int x = 0; x < 5; ++x)
...
TEST(MathTest, convert_scale_abs_uses_even_rounding_and_saturation)
{
...
TEST(MathTest, convert_fp16_covers_special_and_subnormal_values)
{
...
TEST(MathTest, float_math_functions_cover_edges_and_power_sign_rule)
{
...
TEST(MathTest, double_math_functions_use_fixed_accuracy)
{
...
TEST(MathTest, unary_math_and_fp16_support_non_contiguous_roi_and_in_place)
{
...
TEST(MathTest, check_range_reports_first_bad_pixel_and_upper_is_exclusive)
{
...
TEST(MathTest, check_range_and_patch_nans_cover_roi_and_type_errors)
{
...
TEST(MathTest, invalid_math_inputs_throw)
{
    Mat u8({1, 3}, CV_8UC1);
