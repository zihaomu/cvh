#include "test/core/support/array_test_utils.hpp"

TEST(ArrayTest, public_numeric_apis_cover_mat_and_scalar_inputs)
{
    Mat a({1, 7}, CV_8UC1);
    Mat b({1, 7}, CV_8UC1);
...
TEST(ArrayTest, bitwise_apis_use_raw_float_bits)
{
    Mat a({1, 3}, CV_32FC1);
    Mat b({1, 3}, CV_32FC1);
...
TEST(ArrayTest, bitwise_scalar_and_mask_preserve_unselected_pixels)
{
    Mat src({1, 5}, CV_8UC4);
    for (int x = 0; x < 5; ++x)
    {
...
TEST(ArrayTest, inrange_combines_channels_into_single_channel_mask)
{
...
TEST(ArrayTest, inrange_integer_scalar_uses_inclusive_fractional_bounds)
{
...
TEST(ArrayTest, public_ops_support_non_contiguous_roi_and_in_place_output)
{
