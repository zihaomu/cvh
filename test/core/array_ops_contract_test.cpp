...
TEST(ArrayOpsContract_TEST, ui_arithmetic_keeps_short_rows_and_masked_bitwise_on_scalar)
{
    ...
    Mat wide_a({2, 19}, CV_8UC3);
    Mat wide_b({2, 19}, CV_8UC3);
    Mat mask({2, 19}, CV_8UC1);
    wide_a.setTo(Scalar(0xF0, 0xAA, 0x55));
    wide_b.setTo(Scalar(0x0F, 0x33, 0xC0));
    mask.setTo(Scalar::all(255));
    out.release();
    bitwise_and(wide_a, wide_b, out, mask);
    EXPECT_EQ(cpu::last_dispatch_tag(), cpu::DispatchTag::Scalar);
}
