...
TEST(ThresholdTest, binary_and_binary_inv_match_reference)
{
    Mat src({2, 4}, CV_8UC1);
...
TEST(ThresholdTest, supports_multi_channel_u8)
{
    Mat src({2, 2}, CV_8UC3);
    for (int y = 0; y < 2; ++y)
    {
...
TEST(ThresholdTest, non_contiguous_roi_path_matches_reference)
{
...
TEST(ThresholdTest, maxval_is_saturated_to_u8)
{
    Mat src({1, 4}, CV_8UC1);
    src.at<uchar>(0, 0) = 0;
    src.at<uchar>(0, 1) = 1;
...
TEST(ThresholdTest, threshold_dryrun_from_upstream_keeps_input_unchanged)
{
...
TEST(ThresholdTest, throws_on_unsupported_depth_or_type)
{
    Mat out;
...
TEST(ThresholdTest, supports_cv32f_fixed_threshold_types_for_c1_c3_c4)
{
    for (int cn : {1, 3, 4})
    {
        SCOPED_TRACE(cn);
...
TEST(ThresholdTest, cv32f_non_contiguous_roi_and_dryrun_match_reference)
{
...
TEST(ThresholdTest, cv32f_rejects_otsu_and_triangle_flags)
{
...
TEST(ThresholdTest, threshold_with_mask_preserves_unselected_pixels)
{
    Mat src({3, 4}, CV_8UC3);
    fill_pattern(src);
