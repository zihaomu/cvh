...
TEST(WarpAffineTest, nearest_identity_u8_c3_keeps_values)
{
    Mat src({5, 7}, CV_8UC3);
...
TEST(WarpAffineTest, nearest_inverse_translation_constant_border_matches_expected)
{
    Mat src({2, 3}, CV_8UC1);
    src.at<uchar>(0, 0) = 1;
    src.at<uchar>(0, 1) = 2;
...
TEST(WarpAffineTest, linear_half_pixel_center_for_f32_is_expected_average)
{
...
TEST(WarpAffineTest, inverse_map_flag_matches_internal_inverse_path)
{
    Mat src({6, 8}, CV_8UC1);
...
TEST(WarpAffineTest, roi_non_contiguous_input_is_supported)
{
...
TEST(WarpAffineTest, in_place_same_mat_is_supported)
{
    Mat src({4, 5}, CV_8UC1);
...
TEST(WarpAffineTest, constant_border_uses_per_channel_scalar_for_outside_pixels)
{
    Mat src({3, 4}, CV_8UC4);
...
TEST(WarpAffineTest, throws_on_invalid_arguments)
{
    Mat dst;
    Mat empty;
    Mat src({4, 4}, CV_8UC1);
